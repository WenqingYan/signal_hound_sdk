...
def peakDetect(df):
    height = df['power'].quantile(0.9) + 5
    peaks, _ = find_peaks(df['power'].tolist(), height=height)
    #print("peak index is:", peaks)
    freq = df.iloc[peaks, 0]
    #print("Peak Freq is:", freq.tolist())
    return freq.mean()
